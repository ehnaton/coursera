...
def rock_spawner():
    ...
    if score%10==0:
        rock_vel[0] += score/10*.5
        rock_vel[1] += score/10*.5
    
    a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    
    # check where's the nearest rock
    distance = dist(rock_pos, my_ship.get_position())         
    if len(rock_group) <= 11 and distance > my_ship.get_radius()*2.5:
